...
/**
  * Created by francois_lavallee on 29/06/17.
  */
case class easterIslandAlphaSControls(integrationStep: Double = 0.25,
                                 timeStep: Double = 1,
                                 b: Double = 0.002,
                                 d: Double = 0.012,
                                 r: Double = 0.004,
  ...
  def dynamic(state: Vector[Double], control: Vector[Double]) = {
    ...
    def zDot(state: Vector[Double], t: Double) = control(0) * state(0) * state(1) - control(1) * state(0) * (1 - math.exp(-state(2) / (rho * state(0)))) - c * state(2)
